...
The authenticated API supports package synchronization, artifact staging, start/stop, logs, latest normalized telemetry, workflow retrieval, arm/disarm of one declared motion gate, revision rollback, and managed provider services. Package synchronization installs only declared sources/components and verifies node and ROS workspace availability before staging.
